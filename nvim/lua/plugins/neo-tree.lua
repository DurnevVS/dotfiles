...
local create_init = function(state)
  local node = state.tree:get_node()
  local path = ""
  if node.type == "directory" then
    path = node.path .. "/"
  else
    path = string.sub(node.path, 1, node.path:match "^.*()/")
  end
  vim.system { "touch", path .. "__init__.py" }
end
  ...
  opts = {
    ...
    window = {
      ...
      mappings = {
        ...
        ["l"] = open_file,
        ["<right>"] = open_file,
        ["<Leader>i"] = { create_init, desc = "Create __init__.py" },
      },
    },
  },
}
